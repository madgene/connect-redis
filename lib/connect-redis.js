...
function generateKey(sid, prefix) {
  return prefix + sid;
}
...
module.exports = function (session) {
  ...
  function RedisStore (options) {
    ...
    this.genKey = options.genkey || generateKey;
    if (typeof this.genKey !== 'function') {
      throw new TypeError('option genkey must be a function');
    }
  ...
  RedisStore.prototype.get = function (sid, fn) {
    var store = this;
    var psid = store.genKey(sid, store.prefix);
    if (!fn) fn = noop;
    debug('GET "%s"', sid);
  ...
  RedisStore.prototype.destroy = function (sid, fn) {
    sid = this.genKey(sid, this.prefix);
    debug('DEL "%s"', sid);
